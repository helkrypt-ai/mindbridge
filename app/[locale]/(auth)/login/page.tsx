  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <h1 className="text-2xl font-bold text-center text-indigo-700 mb-6">{t("signInTitle")}</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="password" placeholder={tCommon("password")} value={password} onChange={e => setPassword(e.target.value)} required className="w-full border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
        ...
        <p className="mt-4 text-center text-sm text-gray-500">
          {t.rich("noAccount", {
            link: (chunks) => <Link href="/signup" className="text-indigo-600 hover:underline">{chunks}</Link>,
          })}
        </p>
      </div>
    </div>
