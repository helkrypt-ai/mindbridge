          ...
          <>
            ...
            {chartData.length > 1 ? (
              <div className="bg-white rounded-xl border p-4 mb-8">
                <h2 className="text-sm font-medium text-gray-500 mb-4">Mood over time</h2>
              ...
              <div className="bg-indigo-50 rounded-xl border border-indigo-100 p-6 text-center mb-8">
                <p className="text-gray-500 text-sm">Log mood check-ins to see your trend chart.</p>
                <Link href="/checkin" className="mt-3 inline-block text-indigo-600 text-sm font-medium hover:underline">Log your first check-in →</Link>
              </div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[{ href: "/checkin", label: "Mood check-in", icon: "📊" }, { href: "/chat", label: "AI companion", icon: "💬" }, { href: "/journal", label: "Journal", icon: "📝" }].map(({ href, label, icon }) => (
